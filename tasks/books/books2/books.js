...
//     const booksIsAvailable = [];
//     for(let i = 0; i < bookCollection.length; i++){
//         if(isAvailable === bookCollection[i].isAvailable){
//             booksIsAvailable.push(bookCollection[i]);
//         }
//     }

//     return booksIsAvailable;
// }
...
//             publicationBooks.push(bookCollection[i]);
//         }
//     }
...
//   const genreBooks = [];
//   for(let i = 0; i < bookCollection.length; i++){
//     for(let k = 0; k < bookCollection[i].genre.length; k++){
//       // console.log(bookCollection[i].genre[k]);
//     }
//   }
// }
...
function getBooksByChaptersNumber(chapters){
  const chapterBooks = [];
  for(let x of bookCollection){
    if(x.chapters >= chapters) chapterBooks.push(x);
  }

  return chapterBooks;
}

getBooksByChaptersNumber(3);


function getBooksByGenre(genre){
  const booksArrGenre = [];
  for(let x of bookCollection){
    if (x.genre.includes(genre)) booksArrGenre.push(x);
  }
  return booksArrGenre;
}

getBooksByGenre("Gothic");


function getBooksByAmazonRating(rating){
  const ratingBooksArr = [];
  let count = 0;
  for(let x of bookCollection){
    for(let j of x.ratings){
      if(j.source === "Amazon" && j.score >= rating){        
        ratingBooksArr.push(x);
      }
      
    }
  }
  return ratingBooksArr;
}
